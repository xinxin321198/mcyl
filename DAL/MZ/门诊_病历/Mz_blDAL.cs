...
namespace DAL.MZ.门诊_病历
{
    ...
    public interface Mz_blDAL
    {
        ...
        bool deleteByOtherCondition(Mz_bl mz_blParamCondition , OracleConnection con);
        ...
        /// <param name="mz_blParam">要修改的mz_blParam对象</param>
        /// <param name="con">Oracle连接对象（为null时自动生成）</param>
        /// <returns>是否修改成功（影响的行数为1时成功）</returns>
        bool updateById(Mz_bl mz_blParam,OracleConnection con);

        /// <summary>
        /// 根据codeList中的ID修改多条数据（如果需要供事务使用就传入OracleConnection对象，如果不供就传入null，方法自动生成OracleConnection对象）
        /// </summary>
        /// <param name=mz_blParamList">要修改的mz_blParamList对象集合</param>
        /// <param name="con">Oracle连接对象（为null时自动生成）</param>
        /// <returns>是否删除成功（影响的行数等于要修改的对象数量时成功）</returns>
        bool updateByIdBatch(List<Mz_bl> mz_blParamList, OracleConnection con);
        ...
        bool updateByOtherCondition(Mz_bl mz_blParamCondition, Mz_bl mz_blParamUpdate, OracleConnection con);
        ...
        List<object> find(Mz_bl mz_blParam, Model.Common.PageInfo page,Mz_bl mz_blParamLike,List<SortInfo> sortList);
        ...
        int getCount(Mz_bl mz_blParam,Mz_bl mz_blParamLike);

        /// <summary>
        /// 通用的保存方法，可新增或修改，根据用户传入的参数，有ID为修改，没有ID为新增(手动生成OracleConnection对象，此方法用于事务，使用事务必须使用同一个OracleConnection对象)
        /// </summary>
        /// <param name="mz_blParam">要保存或更新的Mz_bl对象</param>
        /// <returns>bool，保存是否成功</returns>
        bool save(Mz_bl mz_blParam,OracleConnection con);
