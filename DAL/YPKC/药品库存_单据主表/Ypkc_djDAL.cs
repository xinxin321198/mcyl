...
namespace DAL.YPKC.药品库存_单据主表
{
    ...
    public interface Ypkc_djDAL
    {
        ...
        /// </summary>
        /// <param name="ypkc_djParam">要插入的Ypkc_dj对象</param>
        /// <param name="con">Oracle连接对象</param>
        /// <returns>是否添加成功（影响的行数为1时成功）</returns>
        bool insert(Ypkc_dj ypkc_djParam, OracleConnection con);
        ...
        bool deleteByOtherCondition(Ypkc_dj ypkc_djParamCondition , OracleConnection con);
        ...
        /// <param name="ypkc_djParam">要修改的ypkc_djParam对象</param>
        /// <param name="con">Oracle连接对象（为null时自动生成）</param>
        /// <returns>是否修改成功（影响的行数为1时成功）</returns>
        bool updateById(Ypkc_dj ypkc_djParam,OracleConnection con);

        /// <summary>
        /// 根据codeList中的ID修改多条数据（如果需要供事务使用就传入OracleConnection对象，如果不供就传入null，方法自动生成OracleConnection对象）
        /// </summary>
        /// <param name=ypkc_djParamList">要修改的ypkc_djParamList对象集合</param>
        /// <param name="con">Oracle连接对象（为null时自动生成）</param>
        /// <returns>是否删除成功（影响的行数等于要修改的对象数量时成功）</returns>
        bool updateByIdBatch(List<Ypkc_dj> ypkc_djParamList, OracleConnection con);
        ...
        bool updateByOtherCondition(Ypkc_dj ypkc_djParamCondition, Ypkc_dj ypkc_djParamUpdate, OracleConnection con);
        ...
        List<object> find(Ypkc_dj ypkc_djParam, Model.Common.PageInfo page,Ypkc_dj ypkc_djParamLike,List<SortInfo> sortList);
        ...
        int getCount(Ypkc_dj ypkc_djParam,Ypkc_dj ypkc_djParamLike);

        /// <summary>
        /// 通用的保存方法，可新增或修改，根据用户传入的参数，有ID为修改，没有ID为新增(手动生成OracleConnection对象，此方法用于事务，使用事务必须使用同一个OracleConnection对象)
        /// </summary>
        /// <param name="ypkc_djParam">要保存或更新的Ypkc_dj对象</param>
        /// <returns>bool，保存是否成功</returns>
        bool save(Ypkc_dj ypkc_djParam,OracleConnection con);
        ...
        /// <returns>Ypkc_dj</returns>
        Ypkc_dj findContactObjectsById(string id);
        ...
        ///<param name="sortList">排序参数对象的集合</param>
        /// <returns>满足条件的所有Ypkc_dj对象集合list</returns>
        List<object> findContactObjects(Ypkc_dj ypkc_djParam, Model.Common.PageInfo page, Ypkc_dj ypkc_djParamLike, List<SortInfo> sortList);
