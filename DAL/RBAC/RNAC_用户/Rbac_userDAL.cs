...
namespace DAL.RBAC.RBAC_用户
{
    ...
    public interface Rbac_userDAL
    {
        ...
        bool deleteByOtherCondition(Rbac_user rbac_userParamCondition , OracleConnection con);
        ...
        /// <param name="rbac_userParam">要修改的rbac_userParam对象</param>
        /// <param name="con">Oracle连接对象（为null时自动生成）</param>
        /// <returns>是否修改成功（影响的行数为1时成功）</returns>
        bool updateById(Rbac_user rbac_userParam,OracleConnection con);

        /// <summary>
        /// 根据codeList中的ID修改多条数据（如果需要供事务使用就传入OracleConnection对象，如果不供就传入null，方法自动生成OracleConnection对象）
        /// </summary>
        /// <param name=rbac_userParamList">要修改的rbac_userParamList对象集合</param>
        /// <param name="con">Oracle连接对象（为null时自动生成）</param>
        /// <returns>是否删除成功（影响的行数等于要修改的对象数量时成功）</returns>
        bool updateByIdBatch(List<Rbac_user> rbac_userParamList, OracleConnection con);
        ...
        bool updateByOtherCondition(Rbac_user rbac_userParamCondition, Rbac_user rbac_userParamUpdate, OracleConnection con);
        ...
        List<object> find(Rbac_user rbac_userParam, Model.Common.PageInfo page,Rbac_user rbac_userParamLike,List<SortInfo> sortList);
        ...
        int getCount(Rbac_user rbac_userParam,Rbac_user rbac_userParamLike);

        /// <summary>
        /// 通用的保存方法，可新增或修改，根据用户传入的参数，有ID为修改，没有ID为新增(手动生成OracleConnection对象，此方法用于事务，使用事务必须使用同一个OracleConnection对象)
        /// </summary>
        /// <param name="rbac_userParam">要保存或更新的Rbac_user对象</param>
        /// <returns>bool，保存是否成功</returns>
        bool save(Rbac_user rbac_userParam,OracleConnection con);
        ...
        /// <returns>Rbac_user</returns>
        Rbac_user findContactObjectsById(string id);
        ...
        ///<param name="sortList">排序参数对象的集合</param>
        /// <returns>满足条件的所有Rbac_user对象集合list</returns>
        List<object> findContactObjects(Rbac_user rbac_userParam, Model.Common.PageInfo page, Rbac_user rbac_userParamLike, List<SortInfo> sortList);
