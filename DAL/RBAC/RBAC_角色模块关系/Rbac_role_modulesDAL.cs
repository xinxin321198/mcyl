...
namespace DAL.RBAC.RBAC_角色模块关系
{
    ...
    /// </summary>
    public interface Rbac_role_modulesDAL
    {
        ...
        /// </summary>
        /// <param name="rbac_role_modulesParam">要插入的Rbac_role_modules对象</param>
        /// <param name="con">Oracle连接对象</param>
        /// <returns>是否添加成功（影响的行数为1时成功）</returns>
        bool insert(Rbac_role_modules rbac_role_modulesParam,OracleConnection con);

        /// <summary>
        /// 插入多条数据（如果需要供事务使用就传入OracleConnection对象，如果不供就传入null，方法自动生成OracleConnection对象）
        /// 主键为null时，如果不指定id,方法自动生成主键guid
        /// </summary>
        /// <param name="rbac_role_modulesParam">要插入的Rbac_role_modules对象的集合</param>
        /// <param name="con">Oracle连接对象（为null时自动生成）</param>
        /// <returns>是否添加成功（影响的行数等于保存的对象的数量时成功）</returns>
        bool inertBatch(List<Rbac_role_modules> rbac_role_modulesParamList, OracleConnection con);
        ...
        bool deleteByOtherCondition(Rbac_role_modules rbac_role_modulesParamCondition , OracleConnection con);
        ...
        /// <param name="rbac_role_modulesParam">要修改的rbac_role_modulesParam对象</param>
        /// <param name="con">Oracle连接对象（为null时自动生成）</param>
        /// <returns>是否修改成功（影响的行数为1时成功）</returns>
        bool updateById(Rbac_role_modules rbac_role_modulesParam,OracleConnection con);

        /// <summary>
        /// 根据codeList中的ID修改多条数据（如果需要供事务使用就传入OracleConnection对象，如果不供就传入null，方法自动生成OracleConnection对象）
        /// </summary>
        /// <param name=rbac_role_modulesParamList">要修改的rbac_role_modulesParamList对象集合</param>
        /// <param name="con">Oracle连接对象（为null时自动生成）</param>
        /// <returns>是否删除成功（影响的行数等于要修改的对象数量时成功）</returns>
        bool updateByIdBatch(List<Rbac_role_modules> rbac_role_modulesParamList, OracleConnection con);
        ...
        bool updateByOtherCondition(Rbac_role_modules rbac_role_modulesParamCondition, Rbac_role_modules rbac_role_modulesParamUpdate, OracleConnection con);
        ...
        Rbac_role_modules findById(string id);
        ...
        List<object> find(Rbac_role_modules rbac_role_modulesParam, Model.Common.PageInfo page,Rbac_role_modules rbac_role_modulesParamLike,List<SortInfo> sortList);
        ...
        int getCount(Rbac_role_modules rbac_role_modulesParam,Rbac_role_modules rbac_role_modulesParamLike);

        /// <summary>
        /// 通用的保存方法，可新增或修改，根据用户传入的参数，有ID为修改，没有ID为新增(手动生成OracleConnection对象，此方法用于事务，使用事务必须使用同一个OracleConnection对象)
        /// </summary>
        /// <param name="rbac_role_modulesParam">要保存或更新的Rbac_role_modules对象</param>
        /// <returns>bool，保存是否成功</returns>
        bool save(Rbac_role_modules rbac_role_modulesParam,OracleConnection con);
        ...
        /// <returns>Rbac_role_modules</returns>
        Rbac_role_modules findContactObjectsById(string id);
        ...
        ///<param name="sortList">排序参数对象的集合</param>
        /// <returns>满足条件的所有Rbac_role_modules对象集合list</returns>
        List<object> findContactObjects(Rbac_role_modules rbac_role_modulesParam, Model.Common.PageInfo page, Rbac_role_modules rbac_role_modulesParamLike, List<SortInfo> sortList);
