...
namespace DAL.RBAC.RBAC_角色
{
    ...
    /// </summary>
    public class Rbac_roleDALImp :Base.BasDAL,Rbac_roleDAL
    {
        ...
        /// </summary>
        /// <param name="rbac_roleParam">要插入的rbac_roleParam对象</param>
        /// <param name="con">Oracle连接对象（为null时自动生成）</param>
        /// <returns>是否添加成功（影响的行数为1时成功）</returns>
        public bool insert(Rbac_role rbac_roleParam,OracleConnection con)
        {
            if (""==rbac_roleParam.Id||null==rbac_roleParam.Id)
            {
                rbac_roleParam.Id = Model.Common.GUIDHelper.getGuid();
            }
            List<OracleParameter> sqlparams = new List<OracleParameter>();
            
            StringBuilder sql = new StringBuilder("INSERT INTO RBAC_ROLE(id,name,remark,zfbz) VALUES(:id,:name,:remark,:zfbz)");
            
            OracleParameter pId = new OracleParameter(":id", OracleDbType.Varchar2);
            pId.Direction = ParameterDirection.Input;
            pId.Value = rbac_roleParam.Id;
            sqlparams.Add(pId);
            OracleParameter pName = new OracleParameter(":name", OracleDbType.NVarchar2);
            pName.Direction = ParameterDirection.Input;
            pName.Value = rbac_roleParam.Name;
            sqlparams.Add(pName);
            OracleParameter pRemark = new OracleParameter(":remark", OracleDbType.NVarchar2);
            pRemark.Direction = ParameterDirection.Input;
            pRemark.Value = rbac_roleParam.Remark;
            ...
            pZfbz.Value = rbac_roleParam.Zfbz;
            sqlparams.Add(pZfbz);
            bool result = false;
        ...
        /// 主键为null时，自动生成主键guid
        /// </summary>
        /// <param name="rbac_roleParamList">要插入的rbac_roleParam对象的集合</param>
        /// <param name="con">Oracle连接对象（为null时自动生成）</param>
        /// <returns>是否添加成功（影响的行数等于保存的对象的数量时成功）</returns>
        public bool inertBatch(List<Rbac_role> rbac_roleParamList, OracleConnection con)
        {
            if (rbac_roleParamList.Count == 0)
            {
                return true;
            }
            ...
            for (int i = 0; i < rbac_roleParamList.Count; i++)
            {
                Rbac_role rbac_roleParam = rbac_roleParamList[i];
                if ("" == rbac_roleParam.Id || null == rbac_roleParam.Id)
                {
                    rbac_roleParam.Id = Model.Common.GUIDHelper.getGuid();
                }

            idList.Add(rbac_roleParam.Id);

            nameList.Add(rbac_roleParam.Name);

            remarkList.Add(rbac_roleParam.Remark);

            zfbzList.Add(rbac_roleParam.Zfbz);
            ...
            bool result = false;
            int flag = -1;
            flag = OracleHelper.ExecuteBatch(sql.ToString(), con, rbac_roleParamList.Count, sqlparams.ToArray());
            if (rbac_roleParamList.Count == flag)
            {
                result  = true;
            }
            return result;
        ...
        public bool deleteById(string id,OracleConnection con)
        {
            ...
            StringBuilder sql = new StringBuilder("DELETE RBAC_ROLE WHERE ID=:id");
            OracleParameter pId = new OracleParameter(":id", OracleDbType.Varchar2);
            pId.Value = id;
            bool result = false;
        ...
        public bool deleteByOtherCondition(Rbac_role rbac_roleParamCondition, OracleConnection con)
        {
            if (null!=rbac_roleParamCondition.Id&&""!=rbac_roleParamCondition.Id)
            {
                throw new Exception("不允许指定参数的ID，如果要根据ID删除数据请调用deleteById或者deleteByIdBatch方法；错误出自：deleteByOtherCondition");
            }
            List<OracleParameter> sqlparams = new List<OracleParameter>();
            StringBuilder sql = new StringBuilder("DELETE RBAC_ROLE ");

            if (null != rbac_roleParamCondition.Name &&  ""!= rbac_roleParamCondition.Name)
            {
                this.isAnd(sql);
                this.isWhere(sql);
                ...
                OracleParameter pName = new OracleParameter(":name", OracleDbType.NVarchar2);
                pName.Direction = ParameterDirection.Input;
                pName.Value = rbac_roleParamCondition.Name;
                sqlparams.Add(pName);

            }
                        
            if (null != rbac_roleParamCondition.Remark &&  ""!= rbac_roleParamCondition.Remark)
            {
                this.isAnd(sql);
                this.isWhere(sql);
                ...
                OracleParameter pRemark = new OracleParameter(":remark", OracleDbType.NVarchar2);
                pRemark.Direction = ParameterDirection.Input;
                pRemark.Value = rbac_roleParamCondition.Remark;
                sqlparams.Add(pRemark);

            }
                        
            if (null != rbac_roleParamCondition.Zfbz &&  ""!= rbac_roleParamCondition.Zfbz)
            {
                this.isAnd(sql);
                this.isWhere(sql);
                ...
                OracleParameter pZfbz = new OracleParameter(":zfbz", OracleDbType.NVarchar2);
                pZfbz.Direction = ParameterDirection.Input;
                pZfbz.Value = rbac_roleParamCondition.Zfbz;
                sqlparams.Add(pZfbz);
        ...
        public bool updateById(Rbac_role rbac_roleParam,OracleConnection con)
        {
            if ("" == rbac_roleParam.Id || null == rbac_roleParam.Id)
            {
            ...
            StringBuilder sql = new StringBuilder("UPDATE RBAC_ROLE SET ");
            if (null != rbac_roleParam.Name &&  ""!= rbac_roleParam.Name)
            {
                this.isComma(sql);
                sql.Append(" NAME=:name ");
                OracleParameter pName = new OracleParameter(":name", OracleDbType.NVarchar2);
            ...
            if (null != rbac_roleParam.Remark &&  ""!= rbac_roleParam.Remark)
            {
                this.isComma(sql);
                sql.Append(" REMARK=:remark ");
                OracleParameter pRemark = new OracleParameter(":remark", OracleDbType.NVarchar2);
            ...
            if (null != rbac_roleParam.Zfbz &&  ""!= rbac_roleParam.Zfbz)
            {
                this.isComma(sql);
                sql.Append(" ZFBZ=:zfbz ");
                OracleParameter pZfbz = new OracleParameter(":zfbz", OracleDbType.NVarchar2);
        ...
        /// </summary>
        /// <param name="rbac_roleParamList">要修改的Rbac_role对象集合</param>
        /// <param name="con">Oracle连接对象（为null时自动生成）</param>
        /// <returns>是否删除成功（影响的行数等于要修改的对象数量时成功）</returns>
        public bool updateByIdBatch(List<Rbac_role> rbac_roleParamList, OracleConnection con)
        {
            ...
            sqlparams.Add(pId);
            bool result = false;
            int flag = OracleHelper.ExecuteBatch(sql.ToString(), con,rbac_roleParamList.Count, sqlparams.ToArray());
            if (rbac_roleParamList.Count == flag)
            {
                result = true;
            }
        ...
        public bool updateByOtherCondition(Rbac_role rbac_roleParamCondition, Rbac_role rbac_roleParamUpdate, OracleConnection con)
        {
            if ((null!=rbac_roleParamCondition.Id&&"!=rbac_roleParamCondition.Id)||(null!=rbac_roleParamUpdate.Id&&"!=rbac_roleParamUpdate.Id))
            {
            ...
            //添加要修改的值
            StringBuilder sql = new StringBuilder("UPDATE RBAC_ROLE SET ");
            if (null != rbac_roleParamUpdate.Name &&  ""!= rbac_roleParamUpdate.Name)
            {
                this.isComma(sql);
                sql.Append(" NAME=:name ");
                OracleParameter pName = new OracleParameter(":name", OracleDbType.NVarchar2);
                pName.Direction = ParameterDirection.Input;
                pName.Value = rbac_roleParamUpdate.Name;
                sqlparams.Add(pName);

            }
                        
            if (null != rbac_roleParamUpdate.Remark &&  ""!= rbac_roleParamUpdate.Remark)
            {
                this.isComma(sql);
                sql.Append(" REMARK=:remark ");
                OracleParameter pRemark = new OracleParameter(":remark", OracleDbType.NVarchar2);
                pRemark.Direction = ParameterDirection.Input;
                pRemark.Value = rbac_roleParamUpdate.Remark;
                sqlparams.Add(pRemark);

            }
                        
            if (null != rbac_roleParamUpdate.Zfbz &&  ""!= rbac_roleParamUpdate.Zfbz)
            {
                this.isComma(sql);
                sql.Append(" ZFBZ=:zfbz ");
                OracleParameter pZfbz = new OracleParameter(":zfbz", OracleDbType.NVarchar2);
                pZfbz.Direction = ParameterDirection.Input;
                pZfbz.Value = rbac_roleParamUpdate.Zfbz;
                sqlparams.Add(pZfbz);
        ...
        public Rbac_role findById(string id)
        {
            if (""==id||null==id)
            {
        ...
        public List<object> find(Rbac_role rbac_roleParam, Model.Common.PageInfo page,Rbac_role rbac_roleParamLike,List<SortInfo> sortList)
        {

            //如果没有传查询条件参数，就new一个空的Rbac_role对象，标示查询所有数据
            if (null == rbac_roleParam)
            {
                rbac_roleParam = new Rbac_role();
            }
            if (null!=rbac_roleParam.Id&&""!=rbac_roleParam.Id)
            {
                throw new Exception("此方法不允许指定参数ID，如果需要根据ID查询数据，请调用findById方法；错误出自：find");
            }
            List<Rbac_role> rbac_roleParamList = new List<Rbac_role>();//接收查询出的Rbac_role对象集合
            List<OracleParameter> sqlparams = new List<OracleParameter>();//保存参数的集合
            StringBuilder sql = new StringBuilder("SELECT A.* ");
            //新建方法findContactObjects(Rbac_role rbac_roleParam, Model.Common.PageInfo page)，在此加入多表连接要显示的项start
            //sql.Append(",B.字段一,B.字段二,B.字段三");
            //新建方法，在此加入多表连接要显示的项end
            sql.Append(" FROM RBAC_ROLE A ");
            //新建一个方法，在此加入多表连接的语句start
            ...
            if(rbac_roleParamLike!=null)
            {

                if (null != rbac_roleParamLike.Name && "" != rbac_roleParamLike.Name)
                {
                    this.isAnd(sql);
                    this.isWhere(sql);
                    ...
                    OracleParameter pName = new OracleParameter(":name", OracleDbType.NVarchar2);
                    pName.Direction = ParameterDirection.Input;
                    pName.Value = rbac_roleParamLike.Name;
                    sqlparams.Add(pName);
                }
                
                if (null != rbac_roleParamLike.Remark && "" != rbac_roleParamLike.Remark)
                {
                    this.isAnd(sql);
                    this.isWhere(sql);
                    ...
                    OracleParameter pRemark = new OracleParameter(":remark", OracleDbType.NVarchar2);
                    pRemark.Direction = ParameterDirection.Input;
                    pRemark.Value = rbac_roleParamLike.Remark;
                    sqlparams.Add(pRemark);
                }
                
                if (null != rbac_roleParamLike.Zfbz && "" != rbac_roleParamLike.Zfbz)
                {
                    this.isAnd(sql);
                    this.isWhere(sql);
                    ...
                    OracleParameter pZfbz = new OracleParameter(":zfbz", OracleDbType.NVarchar2);
                    pZfbz.Direction = ParameterDirection.Input;
                    pZfbz.Value = rbac_roleParamLike.Zfbz;
                    sqlparams.Add(pZfbz);
                }
            ...
            List<object> returnList = new List<object>();//返回的List,索引0为List<Rbac_role>对象,索引1为Model.Common.PageInfo分页信息对象
            if (null != page)
            {
            ...
            DataRowCollection rows = ds.Tables[0].Rows;
            //把每一行数据转换为一个对象放入List<Rbac_role>中
            for (int i = 0; i < rows.Count; i++)
            {
                DataRow row = rows[i];
                //rbac_roleParamList.Add(this.dataSetContactObjectsToModel(row));
                rbac_roleParamList.Add(this.dataSetToModel(row));
            }
            ...
            returnList.Add(rbac_roleParamList);//把List<Rbac_role>放入List<object>中第一个
        ...
        /// </summary>
        /// <returns>int</returns>
        public int getCount(Rbac_role rbac_roleParam,Rbac_role rbac_roleParamLike)
        {
            //如果没有传查询条件参数，就new一个空的Rbac_role对象，标示查询所有数据
            if (null == rbac_roleParam)
            {
            ...
            List<OracleParameter> sqlparams = new List<OracleParameter>();//保存参数的集合
            StringBuilder sql = new StringBuilder("SELECT COUNT(ID) FROM RBAC_ROLE A");
                if (null != rbac_roleParam.Name && "" != rbac_roleParam.Name)
                {
                    this.isAnd(sql);
                    this.isWhere(sql);
...
this.b_isWhere = false;return Int32.Parse(OracleHelper.ExecuteScalar(sql.ToString(),sqlparams.ToArray()).ToString());
        ...
        /// <param name="rbac_roleParam"></param>
        /// <returns></returns>
        public bool save(Rbac_role rbac_roleParam,OracleConnection con)
        {
            if ("" != rbac_roleParam.Id && null != rbac_roleParam.Id)
            {
                //修改
                return this.updateById(rbac_roleParam,con);
            }
            else
            {
                //新增
                return this.insert(rbac_roleParam,con);
            }
        }
        ...
        /// <returns></returns>
        private Rbac_role dataSetToModel(DataRow row)
        {
            Rbac_role rbac_roleParam = new Rbac_role();
            rbac_roleParam.Id = Convert.ToString(row["ID"]).Trim(); 
            rbac_roleParam.Name = Convert.ToString(row["NAME"]).Trim(); 
            rbac_roleParam.Remark = Convert.ToString(row["REMARK"]).Trim(); 
            rbac_roleParam.Zfbz = Convert.ToString(row["ZFBZ"]).Trim(); 
            
            return rbac_roleParam;
        }
