...
namespace DAL.RBAC.RBAC_用户角色关系表
{
    ...
    /// </summary>
    public interface Rbac_user_roleDAL
    {
        ...
        /// </summary>
        /// <param name="rbac_user_roleParam">要插入的Rbac_user_role对象</param>
        /// <param name="con">Oracle连接对象</param>
        /// <returns>是否添加成功（影响的行数为1时成功）</returns>
        bool insert(Rbac_user_role rbac_user_roleParam,OracleConnection con);

        /// <summary>
        /// 插入多条数据（如果需要供事务使用就传入OracleConnection对象，如果不供就传入null，方法自动生成OracleConnection对象）
        /// 主键为null时，如果不指定id,方法自动生成主键guid
        /// </summary>
        /// <param name="rbac_user_roleParam">要插入的Rbac_user_role对象的集合</param>
        /// <param name="con">Oracle连接对象（为null时自动生成）</param>
        /// <returns>是否添加成功（影响的行数等于保存的对象的数量时成功）</returns>
        bool inertBatch(List<Rbac_user_role> rbac_user_roleParamList, OracleConnection con);
        ...
        bool deleteByOtherCondition(Rbac_user_role rbac_user_roleParamCondition , OracleConnection con);
        ...
        /// <param name="rbac_user_roleParam">要修改的rbac_user_roleParam对象</param>
        /// <param name="con">Oracle连接对象（为null时自动生成）</param>
        /// <returns>是否修改成功（影响的行数为1时成功）</returns>
        bool updateById(Rbac_user_role rbac_user_roleParam,OracleConnection con);

        /// <summary>
        /// 根据codeList中的ID修改多条数据（如果需要供事务使用就传入OracleConnection对象，如果不供就传入null，方法自动生成OracleConnection对象）
        /// </summary>
        /// <param name=rbac_user_roleParamList">要修改的rbac_user_roleParamList对象集合</param>
        /// <param name="con">Oracle连接对象（为null时自动生成）</param>
        /// <returns>是否删除成功（影响的行数等于要修改的对象数量时成功）</returns>
        bool updateByIdBatch(List<Rbac_user_role> rbac_user_roleParamList, OracleConnection con);
        ...
        bool updateByOtherCondition(Rbac_user_role rbac_user_roleParamCondition, Rbac_user_role rbac_user_roleParamUpdate, OracleConnection con);
        ...
        Rbac_user_role findById(string id);
        ...
        List<object> find(Rbac_user_role rbac_user_roleParam, Model.Common.PageInfo page,Rbac_user_role rbac_user_roleParamLike,List<SortInfo> sortList);
        ...
        int getCount(Rbac_user_role rbac_user_roleParam,Rbac_user_role rbac_user_roleParamLike);

        /// <summary>
        /// 通用的保存方法，可新增或修改，根据用户传入的参数，有ID为修改，没有ID为新增(手动生成OracleConnection对象，此方法用于事务，使用事务必须使用同一个OracleConnection对象)
        /// </summary>
        /// <param name="rbac_user_roleParam">要保存或更新的Rbac_user_role对象</param>
        /// <returns>bool，保存是否成功</returns>
        bool save(Rbac_user_role rbac_user_roleParam,OracleConnection con);
        ...
        /// <returns>Rbac_user_role</returns>
        Rbac_user_role findContactObjectsById(string id);
        ...
        ///<param name="sortList">排序参数对象的集合</param>
        /// <returns>满足条件的所有Rbac_user_role对象集合list</returns>
        List<object> findContactObjects(Rbac_user_role rbac_user_roleParam, Model.Common.PageInfo page, Rbac_user_role rbac_user_roleParamLike, List<SortInfo> sortList);
