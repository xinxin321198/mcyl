...
namespace DAL.JC.基础_代码类型字典
{
    ...
    /// </summary>
    public interface Jc_code_typeDAL
    {
        ...
        bool deleteByOtherCondition(Jc_code_type jc_code_typeParamCondition , OracleConnection con);
        ...
        /// <param name="jc_code_typeParam">要修改的jc_code_typeParam对象</param>
        /// <param name="con">Oracle连接对象（为null时自动生成）</param>
        /// <returns>是否修改成功（影响的行数为1时成功）</returns>
        bool updateById(Jc_code_type jc_code_typeParam,OracleConnection con);

        /// <summary>
        /// 根据codeList中的ID修改多条数据（如果需要供事务使用就传入OracleConnection对象，如果不供就传入null，方法自动生成OracleConnection对象）
        /// </summary>
        /// <param name=jc_code_typeParamList">要修改的jc_code_typeParamList对象集合</param>
        /// <param name="con">Oracle连接对象（为null时自动生成）</param>
        /// <returns>是否删除成功（影响的行数等于要修改的对象数量时成功）</returns>
        bool updateByIdBatch(List<Jc_code_type> jc_code_typeParamList, OracleConnection con);
        ...
        bool updateByOtherCondition(Jc_code_type jc_code_typeParamCondition, Jc_code_type jc_code_typeParamUpdate, OracleConnection con);
        ...
        Jc_code_type findById(string id);
        ...
        List<object> find(Jc_code_type jc_code_typeParam, Model.Common.PageInfo page,Jc_code_type jc_code_typeParamLike,List<SortInfo> sortList);
        ...
        /// </summary>
        /// <param name="jc_code_typeParam">要保存或更新的Jc_code_type对象</param>
        /// <returns>bool，保存是否成功</returns>
        bool save(Jc_code_type jc_code_typeParam,OracleConnection con);
