...
namespace DAL.JC.基础_用户登录日志
{
    ...
    public interface Jc_loginDAL
    {
        ...
        bool deleteByOtherCondition(Jc_login jc_loginParamCondition , OracleConnection con);
        ...
        /// <param name="jc_loginParam">要修改的jc_loginParam对象</param>
        /// <param name="con">Oracle连接对象（为null时自动生成）</param>
        /// <returns>是否修改成功（影响的行数为1时成功）</returns>
        bool updateById(Jc_login jc_loginParam,OracleConnection con);

        /// <summary>
        /// 根据codeList中的ID修改多条数据（如果需要供事务使用就传入OracleConnection对象，如果不供就传入null，方法自动生成OracleConnection对象）
        /// </summary>
        /// <param name=jc_loginParamList">要修改的jc_loginParamList对象集合</param>
        /// <param name="con">Oracle连接对象（为null时自动生成）</param>
        /// <returns>是否删除成功（影响的行数等于要修改的对象数量时成功）</returns>
        bool updateByIdBatch(List<Jc_login> jc_loginParamList, OracleConnection con);
        ...
        bool updateByOtherCondition(Jc_login jc_loginParamCondition, Jc_login jc_loginParamUpdate, OracleConnection con);
        ...
        List<object> find(Jc_login jc_loginParam, Model.Common.PageInfo page,Jc_login jc_loginParamLike,List<SortInfo> sortList);
        ...
        int getCount(Jc_login jc_loginParam,Jc_login jc_loginParamLike);

        /// <summary>
        /// 通用的保存方法，可新增或修改，根据用户传入的参数，有ID为修改，没有ID为新增(手动生成OracleConnection对象，此方法用于事务，使用事务必须使用同一个OracleConnection对象)
        /// </summary>
        /// <param name="jc_loginParam">要保存或更新的Jc_login对象</param>
        /// <returns>bool，保存是否成功</returns>
        bool save(Jc_login jc_loginParam,OracleConnection con);
