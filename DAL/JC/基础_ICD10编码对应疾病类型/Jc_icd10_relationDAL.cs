...
namespace DAL.JC.基础_ICD10编码对应疾病类型
{
    ...
    /// </summary>
    public interface Jc_icd10_relationDAL
    {
        ...
        /// </summary>
        /// <param name="jc_icd10_relationParam">要插入的Jc_icd10_relation对象</param>
        /// <param name="con">Oracle连接对象</param>
        /// <returns>是否添加成功（影响的行数为1时成功）</returns>
        bool insert(Jc_icd10_relation jc_icd10_relationParam,OracleConnection con);

        /// <summary>
        /// 插入多条数据（如果需要供事务使用就传入OracleConnection对象，如果不供就传入null，方法自动生成OracleConnection对象）
        /// 主键为null时，如果不指定id,方法自动生成主键guid
        /// </summary>
        /// <param name="jc_icd10_relationParam">要插入的Jc_icd10_relation对象的集合</param>
        /// <param name="con">Oracle连接对象（为null时自动生成）</param>
        /// <returns>是否添加成功（影响的行数等于保存的对象的数量时成功）</returns>
        bool inertBatch(List<Jc_icd10_relation> jc_icd10_relationParamList, OracleConnection con);
        ...
        bool deleteByOtherCondition(Jc_icd10_relation jc_icd10_relationParamCondition , OracleConnection con);
        ...
        /// <param name="jc_icd10_relationParam">要修改的jc_icd10_relationParam对象</param>
        /// <param name="con">Oracle连接对象（为null时自动生成）</param>
        /// <returns>是否修改成功（影响的行数为1时成功）</returns>
        bool updateById(Jc_icd10_relation jc_icd10_relationParam,OracleConnection con);

        /// <summary>
        /// 根据codeList中的ID修改多条数据（如果需要供事务使用就传入OracleConnection对象，如果不供就传入null，方法自动生成OracleConnection对象）
        /// </summary>
        /// <param name=jc_icd10_relationParamList">要修改的jc_icd10_relationParamList对象集合</param>
        /// <param name="con">Oracle连接对象（为null时自动生成）</param>
        /// <returns>是否删除成功（影响的行数等于要修改的对象数量时成功）</returns>
        bool updateByIdBatch(List<Jc_icd10_relation> jc_icd10_relationParamList, OracleConnection con);
        ...
        bool updateByOtherCondition(Jc_icd10_relation jc_icd10_relationParamCondition, Jc_icd10_relation jc_icd10_relationParamUpdate, OracleConnection con);
        ...
        Jc_icd10_relation findById(string id);
        ...
        List<object> find(Jc_icd10_relation jc_icd10_relationParam, Model.Common.PageInfo page,Jc_icd10_relation jc_icd10_relationParamLike,List<SortInfo> sortList);
        ...
        int getCount(Jc_icd10_relation jc_icd10_relationParam,Jc_icd10_relation jc_icd10_relationParamLike);

        /// <summary>
        /// 通用的保存方法，可新增或修改，根据用户传入的参数，有ID为修改，没有ID为新增(手动生成OracleConnection对象，此方法用于事务，使用事务必须使用同一个OracleConnection对象)
        /// </summary>
        /// <param name="jc_icd10_relationParam">要保存或更新的Jc_icd10_relation对象</param>
        /// <returns>bool，保存是否成功</returns>
        bool save(Jc_icd10_relation jc_icd10_relationParam,OracleConnection con);
        ...
        /// <returns>Jc_icd10_relation</returns>
        Jc_icd10_relation findContactObjectsById(string id);
        ...
        ///<param name="sortList">排序参数对象的集合</param>
        /// <returns>满足条件的所有Jc_icd10_relation对象集合list</returns>
        List<object> findContactObjects(Jc_icd10_relation jc_icd10_relationParam, Model.Common.PageInfo page, Jc_icd10_relation jc_icd10_relationParamLike, List<SortInfo> sortList);
