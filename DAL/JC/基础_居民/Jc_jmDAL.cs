...
namespace DAL.JC.基础_居民
{
    ...
    public interface Jc_jmDAL
    {
        ...
        bool deleteByOtherCondition(Jc_jm jc_jmParamCondition , OracleConnection con);
        ...
        /// <param name="jc_jmParam">要修改的jc_jmParam对象</param>
        /// <param name="con">Oracle连接对象（为null时自动生成）</param>
        /// <returns>是否修改成功（影响的行数为1时成功）</returns>
        bool updateById(Jc_jm jc_jmParam,OracleConnection con);

        /// <summary>
        /// 根据codeList中的ID修改多条数据（如果需要供事务使用就传入OracleConnection对象，如果不供就传入null，方法自动生成OracleConnection对象）
        /// </summary>
        /// <param name=jc_jmParamList">要修改的jc_jmParamList对象集合</param>
        /// <param name="con">Oracle连接对象（为null时自动生成）</param>
        /// <returns>是否删除成功（影响的行数等于要修改的对象数量时成功）</returns>
        bool updateByIdBatch(List<Jc_jm> jc_jmParamList, OracleConnection con);
        ...
        bool updateByOtherCondition(Jc_jm jc_jmParamCondition, Jc_jm jc_jmParamUpdate, OracleConnection con);
        ...
        List<object> find(Jc_jm jc_jmParam, Model.Common.PageInfo page,Jc_jm jc_jmParamLike,List<SortInfo> sortList);
        ...
        int getCount(Jc_jm jc_jmParam,Jc_jm jc_jmParamLike);

        /// <summary>
        /// 通用的保存方法，可新增或修改，根据用户传入的参数，有ID为修改，没有ID为新增(手动生成OracleConnection对象，此方法用于事务，使用事务必须使用同一个OracleConnection对象)
        /// </summary>
        /// <param name="jc_jmParam">要保存或更新的Jc_jm对象</param>
        /// <returns>bool，保存是否成功</returns>
        bool save(Jc_jm jc_jmParam,OracleConnection con);
        ...
        /// <returns>Jc_jm</returns>
        Jc_jm findContactObjectsById(string id);
        ...
        ///<param name="sortList">排序参数对象的集合</param>
        /// <returns>满足条件的所有Jc_jm对象集合list</returns>
        List<object> findContactObjects(Jc_jm jc_jmParam, Model.Common.PageInfo page, Jc_jm jc_jmParamLike, List<SortInfo> sortList);
