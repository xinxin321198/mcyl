...
namespace DAL.JC.基础_工作人员所属科室关系
{
    ...
    public interface Jc_ry_ksDAL
    {
        ...
        bool deleteByOtherCondition(Jc_ry_ks jc_ry_ksParamCondition , OracleConnection con);
        ...
        /// <param name="jc_ry_ksParam">要修改的jc_ry_ksParam对象</param>
        /// <param name="con">Oracle连接对象（为null时自动生成）</param>
        /// <returns>是否修改成功（影响的行数为1时成功）</returns>
        bool updateById(Jc_ry_ks jc_ry_ksParam,OracleConnection con);

        /// <summary>
        /// 根据codeList中的ID修改多条数据（如果需要供事务使用就传入OracleConnection对象，如果不供就传入null，方法自动生成OracleConnection对象）
        /// </summary>
        /// <param name=jc_ry_ksParamList">要修改的jc_ry_ksParamList对象集合</param>
        /// <param name="con">Oracle连接对象（为null时自动生成）</param>
        /// <returns>是否删除成功（影响的行数等于要修改的对象数量时成功）</returns>
        bool updateByIdBatch(List<Jc_ry_ks> jc_ry_ksParamList, OracleConnection con);
        ...
        bool updateByOtherCondition(Jc_ry_ks jc_ry_ksParamCondition, Jc_ry_ks jc_ry_ksParamUpdate, OracleConnection con);
        ...
        List<object> find(Jc_ry_ks jc_ry_ksParam, Model.Common.PageInfo page,Jc_ry_ks jc_ry_ksParamLike,List<SortInfo> sortList);
        ...
        int getCount(Jc_ry_ks jc_ry_ksParam,Jc_ry_ks jc_ry_ksParamLike);

        /// <summary>
        /// 通用的保存方法，可新增或修改，根据用户传入的参数，有ID为修改，没有ID为新增(手动生成OracleConnection对象，此方法用于事务，使用事务必须使用同一个OracleConnection对象)
        /// </summary>
        /// <param name="jc_ry_ksParam">要保存或更新的Jc_ry_ks对象</param>
        /// <returns>bool，保存是否成功</returns>
        bool save(Jc_ry_ks jc_ry_ksParam,OracleConnection con);
        ...
        /// <returns>Jc_ry_ks</returns>
        Jc_ry_ks findContactObjectsById(string id);
        ...
        ///<param name="sortList">排序参数对象的集合</param>
        /// <returns>满足条件的所有Jc_ry_ks对象集合list</returns>
        List<object> findContactObjects(Jc_ry_ks jc_ry_ksParam, Model.Common.PageInfo page, Jc_ry_ks jc_ry_ksParamLike, List<SortInfo> sortList);
