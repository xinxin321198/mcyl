...
namespace DAL.JC.基础_ICD10
{
    ...
    public interface Jc_icd10DAL
    {
        ...
        bool deleteByOtherCondition(Jc_icd10 jc_icd10ParamCondition , OracleConnection con);
        ...
        /// <param name="jc_icd10Param">要修改的jc_icd10Param对象</param>
        /// <param name="con">Oracle连接对象（为null时自动生成）</param>
        /// <returns>是否修改成功（影响的行数为1时成功）</returns>
        bool updateById(Jc_icd10 jc_icd10Param,OracleConnection con);

        /// <summary>
        /// 根据codeList中的ID修改多条数据（如果需要供事务使用就传入OracleConnection对象，如果不供就传入null，方法自动生成OracleConnection对象）
        /// </summary>
        /// <param name=jc_icd10ParamList">要修改的jc_icd10ParamList对象集合</param>
        /// <param name="con">Oracle连接对象（为null时自动生成）</param>
        /// <returns>是否删除成功（影响的行数等于要修改的对象数量时成功）</returns>
        bool updateByIdBatch(List<Jc_icd10> jc_icd10ParamList, OracleConnection con);
        ...
        bool updateByOtherCondition(Jc_icd10 jc_icd10ParamCondition, Jc_icd10 jc_icd10ParamUpdate, OracleConnection con);
        ...
        List<object> find(Jc_icd10 jc_icd10Param, Model.Common.PageInfo page,Jc_icd10 jc_icd10ParamLike,List<SortInfo> sortList);
        ...
        int getCount(Jc_icd10 jc_icd10Param,Jc_icd10 jc_icd10ParamLike);

        /// <summary>
        /// 通用的保存方法，可新增或修改，根据用户传入的参数，有ID为修改，没有ID为新增(手动生成OracleConnection对象，此方法用于事务，使用事务必须使用同一个OracleConnection对象)
        /// </summary>
        /// <param name="jc_icd10Param">要保存或更新的Jc_icd10对象</param>
        /// <returns>bool，保存是否成功</returns>
        bool save(Jc_icd10 jc_icd10Param,OracleConnection con);


        //*******************在此以后添加新的方法start
        /// <summary>
        /// 根据用户传入的参数查询,字母匹配ICD10及拼音码，中文名称匹配名称（id参数排除在外，如需查询id请使用findById方法）
        /// 第一个参数是查询参数，如果查询参数为null则查询所有
        /// 第二个参数是分页参数，传入带数据总行数的分页对象得到默认前PageSiz条数据（取决于page对象中的值），如果传null，则查出所有数据
        /// 第三个参数提供模糊查询功能，只适用于string类型的字段
        /// 第四个参数是排序的参数，传入排序的对象的集合，如果传null，则默认使用ID字段进行排序
        ...
        ///<param name="sortList">排序参数对象的集合</param>
        /// <returns>满足条件的所有Jc_icd10对象集合list</returns>
        List<object> findForFast(Jc_icd10 jc_icd10Param, Model.Common.PageInfo page, Jc_icd10 jc_icd10ParamLike, List<SortInfo> sortList);
