...
namespace DAL.JC.基础_行政区划
{
    ...
    public interface Jc_areaDAL
    {
        ...
        bool deleteByOtherCondition(Jc_area jc_areaParamCondition , OracleConnection con);
        ...
        /// <param name="jc_areaParam">要修改的jc_areaParam对象</param>
        /// <param name="con">Oracle连接对象（为null时自动生成）</param>
        /// <returns>是否修改成功（影响的行数为1时成功）</returns>
        bool updateById(Jc_area jc_areaParam,OracleConnection con);

        /// <summary>
        /// 根据codeList中的ID修改多条数据（如果需要供事务使用就传入OracleConnection对象，如果不供就传入null，方法自动生成OracleConnection对象）
        /// </summary>
        /// <param name=jc_areaParamList">要修改的jc_areaParamList对象集合</param>
        /// <param name="con">Oracle连接对象（为null时自动生成）</param>
        /// <returns>是否删除成功（影响的行数等于要修改的对象数量时成功）</returns>
        bool updateByIdBatch(List<Jc_area> jc_areaParamList, OracleConnection con);
        ...
        bool updateByOtherCondition(Jc_area jc_areaParamCondition, Jc_area jc_areaParamUpdate, OracleConnection con);
        ...
        List<object> find(Jc_area jc_areaParam, Model.Common.PageInfo page,Jc_area jc_areaParamLike,List<SortInfo> sortList);
        ...
        int getCount(Jc_area jc_areaParam,Jc_area jc_areaParamLike);

        /// <summary>
        /// 通用的保存方法，可新增或修改，根据用户传入的参数，有ID为修改，没有ID为新增(手动生成OracleConnection对象，此方法用于事务，使用事务必须使用同一个OracleConnection对象)
        /// </summary>
        /// <param name="jc_areaParam">要保存或更新的Jc_area对象</param>
        /// <returns>bool，保存是否成功</returns>
        bool save(Jc_area jc_areaParam,OracleConnection con);
        ...
        /// <returns>Jc_area</returns>
        Jc_area findContactObjectsById(string id);
        ...
        ///<param name="sortList">排序参数对象的集合</param>
        /// <returns>满足条件的所有Jc_area对象集合list</returns>
        List<object> findContactObjects(Jc_area jc_areaParam, Model.Common.PageInfo page, Jc_area jc_areaParamLike, List<SortInfo> sortList);


        #endregion DAL层的interface,接口，end
        //*******************在此以后添加新的方法end

        bool deleteCurrentAndChildNode(string id);
