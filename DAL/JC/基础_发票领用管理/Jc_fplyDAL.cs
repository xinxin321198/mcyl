...
namespace DAL.JC.基础_发票领用管理
{
    ...
    public interface Jc_fplyDAL
    {
        ...
        bool deleteByOtherCondition(Jc_fply jc_fplyParamCondition , OracleConnection con);
        ...
        /// <param name="jc_fplyParam">要修改的jc_fplyParam对象</param>
        /// <param name="con">Oracle连接对象（为null时自动生成）</param>
        /// <returns>是否修改成功（影响的行数为1时成功）</returns>
        bool updateById(Jc_fply jc_fplyParam,OracleConnection con);

        /// <summary>
        /// 根据codeList中的ID修改多条数据（如果需要供事务使用就传入OracleConnection对象，如果不供就传入null，方法自动生成OracleConnection对象）
        /// </summary>
        /// <param name=jc_fplyParamList">要修改的jc_fplyParamList对象集合</param>
        /// <param name="con">Oracle连接对象（为null时自动生成）</param>
        /// <returns>是否删除成功（影响的行数等于要修改的对象数量时成功）</returns>
        bool updateByIdBatch(List<Jc_fply> jc_fplyParamList, OracleConnection con);
        ...
        bool updateByOtherCondition(Jc_fply jc_fplyParamCondition, Jc_fply jc_fplyParamUpdate, OracleConnection con);
        ...
        List<object> find(Jc_fply jc_fplyParam, Model.Common.PageInfo page,Jc_fply jc_fplyParamLike,List<SortInfo> sortList);
        ...
        /// </summary>
        /// <param name="jc_fplyParam">要保存或更新的Jc_fply对象</param>
        /// <returns>bool，保存是否成功</returns>
        bool save(Jc_fply jc_fplyParam,OracleConnection con);
