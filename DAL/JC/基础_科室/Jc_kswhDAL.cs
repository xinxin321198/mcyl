...
namespace DAL.JC.基础_科室
{
    ...
    public interface Jc_kswhDAL
    {
        ...
        bool deleteByOtherCondition(Jc_kswh jc_kswhParamCondition , OracleConnection con);
        ...
        /// <param name="jc_kswhParam">要修改的jc_kswhParam对象</param>
        /// <param name="con">Oracle连接对象（为null时自动生成）</param>
        /// <returns>是否修改成功（影响的行数为1时成功）</returns>
        bool updateById(Jc_kswh jc_kswhParam,OracleConnection con);

        /// <summary>
        /// 根据codeList中的ID修改多条数据（如果需要供事务使用就传入OracleConnection对象，如果不供就传入null，方法自动生成OracleConnection对象）
        /// </summary>
        /// <param name=jc_kswhParamList">要修改的jc_kswhParamList对象集合</param>
        /// <param name="con">Oracle连接对象（为null时自动生成）</param>
        /// <returns>是否删除成功（影响的行数等于要修改的对象数量时成功）</returns>
        bool updateByIdBatch(List<Jc_kswh> jc_kswhParamList, OracleConnection con);
        ...
        bool updateByOtherCondition(Jc_kswh jc_kswhParamCondition, Jc_kswh jc_kswhParamUpdate, OracleConnection con);
        ...
        List<object> find(Jc_kswh jc_kswhParam, Model.Common.PageInfo page,Jc_kswh jc_kswhParamLike,List<SortInfo> sortList);
        ...
        int getCount(Jc_kswh jc_kswhParam,Jc_kswh jc_kswhParamLike);

        /// <summary>
        /// 通用的保存方法，可新增或修改，根据用户传入的参数，有ID为修改，没有ID为新增(手动生成OracleConnection对象，此方法用于事务，使用事务必须使用同一个OracleConnection对象)
        /// </summary>
        /// <param name="jc_kswhParam">要保存或更新的Jc_kswh对象</param>
        /// <returns>bool，保存是否成功</returns>
        bool save(Jc_kswh jc_kswhParam,OracleConnection con);
        ...
        /// <returns>Jc_kswh</returns>
        Jc_kswh findContactObjectsById(string id);
        ...
        ///<param name="sortList">排序参数对象的集合</param>
        /// <returns>满足条件的所有Jc_kswh对象集合list</returns>
        List<object> findContactObjects(Jc_kswh jc_kswhParam, Model.Common.PageInfo page, Jc_kswh jc_kswhParamLike, List<SortInfo> sortList);
