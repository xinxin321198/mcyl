...
namespace DAL.JC.基础_药品字典
{
    ...
    public interface Jc_ypDAL
    {
        ...
        bool deleteByOtherCondition(Jc_yp jc_ypParamCondition , OracleConnection con);
        ...
        /// <param name="jc_ypParam">要修改的jc_ypParam对象</param>
        /// <param name="con">Oracle连接对象（为null时自动生成）</param>
        /// <returns>是否修改成功（影响的行数为1时成功）</returns>
        bool updateById(Jc_yp jc_ypParam,OracleConnection con);

        /// <summary>
        /// 根据codeList中的ID修改多条数据（如果需要供事务使用就传入OracleConnection对象，如果不供就传入null，方法自动生成OracleConnection对象）
        /// </summary>
        /// <param name=jc_ypParamList">要修改的jc_ypParamList对象集合</param>
        /// <param name="con">Oracle连接对象（为null时自动生成）</param>
        /// <returns>是否删除成功（影响的行数等于要修改的对象数量时成功）</returns>
        bool updateByIdBatch(List<Jc_yp> jc_ypParamList, OracleConnection con);
        ...
        bool updateByOtherCondition(Jc_yp jc_ypParamCondition, Jc_yp jc_ypParamUpdate, OracleConnection con);
        ...
        List<object> find(Jc_yp jc_ypParam, Model.Common.PageInfo page,Jc_yp jc_ypParamLike,List<SortInfo> sortList);
        ...
        int getCount(Jc_yp jc_ypParam,Jc_yp jc_ypParamLike);        
        ...
        /// </summary>
        /// <returns>int,数量</returns>
        int getCountForFast(Jc_yp jc_ypParam, Jc_yp jc_ypParamLike);
        
        /// <summary>
        /// 通用的保存方法，可新增或修改，根据用户传入的参数，有ID为修改，没有ID为新增(手动生成OracleConnection对象，此方法用于事务，使用事务必须使用同一个OracleConnection对象)
        /// </summary>
        /// <param name="jc_ypParam">要保存或更新的Jc_yp对象</param>
        /// <returns>bool，保存是否成功</returns>
        bool save(Jc_yp jc_ypParam,OracleConnection con);
        ...
        /// <returns>Jc_yp</returns>
        Jc_yp findContactObjectsById(string id);
        ...
        ///<param name="sortList">排序参数对象的集合</param>
        /// <returns>满足条件的所有Jc_yp对象集合list</returns>
        List<object> findContactObjects(Jc_yp jc_ypParam, Model.Common.PageInfo page, Jc_yp jc_ypParamLike, List<SortInfo> sortList);

        /// <summary>
        //查询编码时查编码首位匹配的，查询名称时匹配名称和通用名称字段
        /// 根据用户传入的参数查询针对快速查询，（id参数排除在外，如需查询id请使用findById方法）
        /// 第一个参数是查询参数，如果查询参数为null则查询所有
        /// 第二个参数是分页参数，传入带数据总行数的分页对象得到默认前PageSiz条数据（取决于page对象中的值），如果传null，则查出所有数据
        /// 第三个参数提供模糊查询功能，只适用于string类型的字段
        /// 第四个参数是排序的参数，传入排序的对象的集合，如果传null，则默认使用ID字段进行排序
        ...
        ///<param name="sortList">排序参数对象的集合</param>
        /// <returns>满足条件的所有Jc_yp对象集合list</returns>
        List<object> findContactObjectsForFast(Jc_yp jc_ypParam, Model.Common.PageInfo page, Jc_yp jc_ypParamLike, List<SortInfo> sortList);
