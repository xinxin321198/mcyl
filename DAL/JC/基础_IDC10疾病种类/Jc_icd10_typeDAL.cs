...
namespace DAL.JC.基础_IDC10疾病种类
{
    ...
    /// </summary>
    public interface Jc_icd10_typeDAL
    {
        ...
        /// </summary>
        /// <param name="jc_icd10_typeParam">要插入的Jc_icd10_type对象</param>
        /// <param name="con">Oracle连接对象</param>
        /// <returns>是否添加成功（影响的行数为1时成功）</returns>
        bool insert(Jc_icd10_type jc_icd10_typeParam,OracleConnection con);

        /// <summary>
        /// 插入多条数据（如果需要供事务使用就传入OracleConnection对象，如果不供就传入null，方法自动生成OracleConnection对象）
        /// 主键为null时，如果不指定id,方法自动生成主键guid
        /// </summary>
        /// <param name="jc_icd10_typeParam">要插入的Jc_icd10_type对象的集合</param>
        /// <param name="con">Oracle连接对象（为null时自动生成）</param>
        /// <returns>是否添加成功（影响的行数等于保存的对象的数量时成功）</returns>
        bool inertBatch(List<Jc_icd10_type> jc_icd10_typeParamList, OracleConnection con);
        ...
        bool deleteByOtherCondition(Jc_icd10_type jc_icd10_typeParamCondition , OracleConnection con);
        ...
        /// <param name="jc_icd10_typeParam">要修改的jc_icd10_typeParam对象</param>
        /// <param name="con">Oracle连接对象（为null时自动生成）</param>
        /// <returns>是否修改成功（影响的行数为1时成功）</returns>
        bool updateById(Jc_icd10_type jc_icd10_typeParam,OracleConnection con);

        /// <summary>
        /// 根据codeList中的ID修改多条数据（如果需要供事务使用就传入OracleConnection对象，如果不供就传入null，方法自动生成OracleConnection对象）
        /// </summary>
        /// <param name=jc_icd10_typeParamList">要修改的jc_icd10_typeParamList对象集合</param>
        /// <param name="con">Oracle连接对象（为null时自动生成）</param>
        /// <returns>是否删除成功（影响的行数等于要修改的对象数量时成功）</returns>
        bool updateByIdBatch(List<Jc_icd10_type> jc_icd10_typeParamList, OracleConnection con);
        ...
        bool updateByOtherCondition(Jc_icd10_type jc_icd10_typeParamCondition, Jc_icd10_type jc_icd10_typeParamUpdate, OracleConnection con);
        ...
        Jc_icd10_type findById(string id);
        ...
        List<object> find(Jc_icd10_type jc_icd10_typeParam, Model.Common.PageInfo page,Jc_icd10_type jc_icd10_typeParamLike,List<SortInfo> sortList);
        ...
        int getCount(Jc_icd10_type jc_icd10_typeParam,Jc_icd10_type jc_icd10_typeParamLike);

        /// <summary>
        /// 通用的保存方法，可新增或修改，根据用户传入的参数，有ID为修改，没有ID为新增(手动生成OracleConnection对象，此方法用于事务，使用事务必须使用同一个OracleConnection对象)
        /// </summary>
        /// <param name="jc_icd10_typeParam">要保存或更新的Jc_icd10_type对象</param>
        /// <returns>bool，保存是否成功</returns>
        bool save(Jc_icd10_type jc_icd10_typeParam,OracleConnection con);
