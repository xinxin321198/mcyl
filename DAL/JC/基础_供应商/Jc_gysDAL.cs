...
namespace DAL.JC.基础_供应商
{
    ...
    public interface Jc_gysDAL
    {
        ...
        bool deleteByOtherCondition(Jc_gys jc_gysParamCondition , OracleConnection con);
        ...
        /// <param name="jc_gysParam">要修改的jc_gysParam对象</param>
        /// <param name="con">Oracle连接对象（为null时自动生成）</param>
        /// <returns>是否修改成功（影响的行数为1时成功）</returns>
        bool updateById(Jc_gys jc_gysParam,OracleConnection con);

        /// <summary>
        /// 根据codeList中的ID修改多条数据（如果需要供事务使用就传入OracleConnection对象，如果不供就传入null，方法自动生成OracleConnection对象）
        /// </summary>
        /// <param name=jc_gysParamList">要修改的jc_gysParamList对象集合</param>
        /// <param name="con">Oracle连接对象（为null时自动生成）</param>
        /// <returns>是否删除成功（影响的行数等于要修改的对象数量时成功）</returns>
        bool updateByIdBatch(List<Jc_gys> jc_gysParamList, OracleConnection con);
        ...
        bool updateByOtherCondition(Jc_gys jc_gysParamCondition, Jc_gys jc_gysParamUpdate, OracleConnection con);
        ...
        List<object> find(Jc_gys jc_gysParam, Model.Common.PageInfo page,Jc_gys jc_gysParamLike,List<SortInfo> sortList);
        ...
        int getCount(Jc_gys jc_gysParam,Jc_gys jc_gysParamLike);

        /// <summary>
        /// 通用的保存方法，可新增或修改，根据用户传入的参数，有ID为修改，没有ID为新增(手动生成OracleConnection对象，此方法用于事务，使用事务必须使用同一个OracleConnection对象)
        /// </summary>
        /// <param name="jc_gysParam">要保存或更新的Jc_gys对象</param>
        /// <returns>bool，保存是否成功</returns>
        bool save(Jc_gys jc_gysParam,OracleConnection con);
        ...
        /// <returns>Jc_gys</returns>
        Jc_gys findContactObjectsById(string id);
        ...
        ///<param name="sortList">排序参数对象的集合</param>
        /// <returns>满足条件的所有Jc_gys对象集合list</returns>
        List<object> findContactObjects(Jc_gys jc_gysParam, Model.Common.PageInfo page, Jc_gys jc_gysParamLike, List<SortInfo> sortList);
